...
public:
    inline constexpr static XY from_bits(u8 xy) { return xy; }
...
inline constexpr XY operator+(const X x, const Y y)
{
    return XY::from_bits(x.value | y.value);
}
...
struct XYwithin
{
    u8 position;
    u8 boundX;
    inline constexpr XYwithin(XY position, XY bound): position(position.value), boundX(bound.x().value) {}
    inline constexpr XY operator*() { return XY::from_bits(position); }
    inline constexpr void operator++() 
    { 
        if ((position & ((u8) 0b111)) == boundX)
        {
            position = (position & (0b111000)) + Y1.value;
        }
        else 
        {
            position++;
    ...
    inline constexpr bool operator!=(const XYwithin other) const { return position != other.position || boundX != other.boundX; }
};
